...
def calculate_r2(
    ...
    for test, train, train_correlation_matrix_complete in transformed_data:
        ...
        high_corr_features = [
            ...
            if ((abs(correlation_coefficient) > 0.7) and (feature not in target_feature_name))
        ]
        assert target_feature_name not in high_corr_features
        high_correlated_features = high_correlated_features.union(set(high_corr_features))
