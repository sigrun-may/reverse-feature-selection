...
def insufficient_results_study_pruner(trial, warm_up_steps, threshold):
    """TODO: add docstring."""
    # pruning of complete study
    if trial.number >= warm_up_steps:
        try:
        ...
        if max(evaluation_metrics_of_completed_trials) < threshold:
            print("no sufficient result after warm_up_steps steps")
            trial.study.stop()
            raise optuna.TrialPruned()
    ...
    if trial.number >= warm_up_steps:
        ...
        if len(evaluation_metrics_of_completed_trials) > patience:
            ...
            if best_value_before_patience + epsilon > best_value_within_patience:
                print("best_value_before_patience")
                trial.study.stop()
                raise optuna.TrialPruned()


def study_no_improvement_pruner(trial, epsilon, warm_up_steps, number_of_similar_best_values, threshold):
    """TODO: add docstring."""
