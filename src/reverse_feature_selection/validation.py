...
def evaluate_feature_selection(
    ...
    for feature_selection_algorithm, result_dict in feature_selection_result_dict.items():
        ...
        for embedded_feature_selection_method, importance_matrix in result_dict.items():
            ...
            raw_robustness = feature_selection_dict["raw_importances"].sum(axis=0)  # / importance_matrix.shape[0]
            print("raw_robustness", raw_robustness[raw_robustness.nonzero()])
            print("max folds ", feature_selection_dict["raw_importances"].shape[0])
            ...
            feature_selection_dict["weighted_feature_importance"] = cumulated_feature_importance
    ...
    for feature_selection_dict, test_data, train_data in feature_selection_result:
        ...
        for selection_method, selected_features in feature_selection_dict.items():
            ...
            # save reverse selection methods
                ...
                (weighted_features, reverse_robustness,) = _calculate_weights_reverse_feature_selection(
                    selected_features,
                    threshold=meta_data_dict["validation"]["reverse_threshold"],
                )
                assert len(weighted_features) == len(reverse_robustness) == len(meta_data_dict["data"]["columns"]) - 1
...
def calculate_micro_metrics(predicted_classes, true_classes, meta_data_dict):
    ...
    return {
        ...
        "balanced_accuracy_score": balanced_accuracy_score(true_classes, predicted_classes),
    }
