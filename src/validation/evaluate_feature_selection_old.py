...
def extract_feature_importance_matrix(
    ...
    for i, cv_iteration_result_df in enumerate(feature_selection_result_cv_list):
        ...
        feature_importance_matrix[i, :] = get_selected_feature_subset(cv_iteration_result_df, method, p_value).values
...
def get_selected_feature_subset(cv_fold_result_df: pd.DataFrame, method: str, p_value: str = None) -> pd.Series:
    if method == "standard":
